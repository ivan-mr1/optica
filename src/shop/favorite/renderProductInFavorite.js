  ...
  arr.forEach((card) => {
    ...
    const cardItem = `
      <article
        ...
        id="${id}"
      >
        <div class="favorite-product__image">
          <img src="${image}" width="100" height="100" alt="${model}" loading="lazy" class="favorite-product__img" />
        </div>
        <div class="favorite-product__wrapper">
          <h3 class="favorite-product__title">${model}</h3>
          <div class="favorite-product__inner">
            <div class="favorite-product__article">Art: ${article}</div>
            <div class="favorite-product__price">${price} грн</div>
          </div>
        </div>
        <button
          data-favorite-remove-btn
          class="favorite-product__delete"
